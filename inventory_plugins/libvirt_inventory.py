...
class InventoryModule(BaseInventoryPlugin, Constructable, Cacheable):
    ...
    def _set_libvirt_vars(self, host, dom):
        self.inventory.set_variable(host, 'libvirt_uuid', dom.UUIDString())

        for varname, mdkey in METADATA_KEYS.items():
            try:
                mdval = dom.metadata(mdkey, None)
                self.inventory.set_variable(host, varname, mdval)
            except libvirt.libvirtError:
                continue
